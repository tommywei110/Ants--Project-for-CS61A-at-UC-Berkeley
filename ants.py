    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.container and Ant.can_contain(self.ant,insect):
                    self.ant.contain_ant(insect)
                elif insect.container and Ant.can_contain(insect,self.ant):
                    insect.contain_ant(self.ant) # gaurenteed that insect is a BodyGuardAnt
                    self.ant = insect
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(self.ant,QueenAnt):
                if self.ant.is_imposter:
                    self.ant = None
            elif isinstance(insect,QueenAnt):
                if insect.is_imposter:
                ...
                if self.ant.container and self.ant.ant is insect:
                    if not isinstance(self.ant.ant,QueenAnt):
                        self.ant.ant = None
                else:
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self,hive):
        distance = 0
        current_place = self.place
        while type(current_place) is not Hive:
            if len(current_place.bees) != 0 and distance >= self.min_range:
                return random_or_none(current_place.bees)
            distance += 1
            current_place = current_place.entrance
    # END Problem 6
...
class ShortThrower(ThrowerAnt):
    ...
    implemented = True   # Change to True to view in the GUI
    def nearest_bee(self,hive):
        the_bee = ThrowerAnt.nearest_bee(self,hive)
        if the_bee == None:
            return
        else:
            current_place = self.place
            distance = 0
            while type(current_place) is not Hive:
                if the_bee in current_place.bees:
                    break
                current_place = current_place.entrance
                distance += 1
...
# END Problem 13
    ...
    def __init__(self):
        ...
        self.list_of_ant_damage_doubled = []
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            current_place = self.place
            # while the there is an exit for the current place
            while(current_place.exit != None):
                # move current_place to the exit
                current_place = current_place.exit
                # if there is an ant at current place
                if isinstance(current_place.ant,Ant):
                    # if this ant is not in list_of_ant_damage_doubled
                    try:
                        sub_ant = current_place.ant.ant
                        if not sub_ant in self.list_of_ant_damage_doubled:
                            sub_ant.damage *= 2
                            self.list_of_ant_damage_doubled.append(sub_ant)
                    except:
                        if not current_place.ant in self.list_of_ant_damage_doubled:
                            current_place.ant.damage *= 2
                            self.list_of_ant_damage_doubled.append(current_place.ant)
        # END Problem 13
...
def make_slow(action):
    ...
    """
    # BEGIN Problem ECdef slow_action(colony):
    def slow_action(colony):
        if colony.time % 2 == 0: # colony.time is even and does nothing on other turns.
            action(colony) # performs the original action on turns
    return slow_action # a new action method
    # END Problem EC
...
def make_stun(action):
    ...
    return stun_action # returns a new action method that does nothing.
    # END Problem EC
...
def apply_effect(effect, bee, duration):
    ...
    changed_action = effect(original_action) #uses the effect on the Bee's .action method to produce a new action method
    def apply_action(colony):
        nonlocal duration #using nonlocal because i need to modify the variable duration, and it is declared in the parent of this function
        if duration <= 0: # if duration is 0 (just in case, -1 or below),
            original_action(colony) #then execute the original action
        else:
            changed_action(colony) #other wise execute the changed action
        duration -= 1 # decrement duration
    bee.action = apply_action # whenever this is called, it will decrement duration and execute the corresponding action
